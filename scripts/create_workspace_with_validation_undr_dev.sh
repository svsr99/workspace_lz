...
hdfs dfs -getfacl $v_dir >> check_hdfs.log
 cron_kinit=` cat ${crontab_file} | grep "/platform/landingzone_utils/jobutils/kinit_initializer.sh" | wc -l`
         if [[ $cron_kinit -le 0 ]]; then
                printf "Script does not exists... Will add to crontab file"
                echo "${kinit_script}" >> "${crontab_file}"
                printf "ADDED\n"
          else
                printf "Already exist...FOUND\n"
          fi
...
local v_type=$1 # CHMOD or CHOWN
local v_hdfs=$2 #DATA_RAW , DATA_REFINED GROUP USER
local v_user=$3 # USER
local v_group=$4 # GROUP
local v_perm=$5 # Permission
local v_local=$6 # 
local v_appid=$7

log $INFO "Validating $v_type on the $v_dir "
if [[ $v_type = "CHMOD" ]] then;
